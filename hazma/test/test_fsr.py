...
def test_scalar_mediator_fsr():
    ...
    [scalar_mediator_fsr.fermion(eng_gams[i], 1000., muon_mass)
     for i in range(100)]


def test_pseudo_scalar_mediator_fsr():
    eng_gams = np.logspace(-3., 3., num=100)
    [pseudo_scalar_mediator_fsr.fermion(eng_gams[i], 1000., muon_mass)
     for i in range(100)]


def test_vector_mediator_fsr():
    eng_gams = np.logspace(-3., 3., num=100)
    [vector_mediator_fsr.fermion(eng_gams[i], 1000., muon_mass)
     for i in range(100)]
