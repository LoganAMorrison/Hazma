...
def muon(positron_energies, muon_energy):
    """
    Returns the positron spectrum from a the decay muon.
...
def positron_decay(particles, cme, positron_energies,
                   mat_elem_sqrd=lambda k_list: 1.0, num_ps_pts=1000,
                   num_bins=25, verbose=False):
    ...
    Examples
    --------
    Example of generating a spectrum from a muon, and two charged pions
    ...
    if hasattr(positron_energies, "__len__"):
        return positron(particles, cme, positron_energies,
                        mat_elem_sqrd=mat_elem_sqrd, num_ps_pts=num_ps_pts,
                        num_bins=num_bins, verbose=verbose)
    return positron_point(particles, cme, positron_energies, mat_elem_sqrd,
                          num_ps_pts, num_bins)
