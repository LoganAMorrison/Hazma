...
cdef double __integrand(double cl, double eng_p, double eng_s,
    ...
    # TODO: Make sure these extra factors of 2 are correct. My reasoning is
    # that the electron and positron are distinguishable and so you
    # need to integrate theta_L over (0, 2pi).
    if fs == "total":
        dnde_cp = 4. * pwpipi * __interp_spec(eng_p_srf, "cp")
        dnde_mu = 4. * pwmumu * __interp_spec(eng_p_srf, "mu")
...
cdef double __dnde_decay_s(double eng_p, double eng_s, double ms,
    ...
    if eminus <= eng_p <= eplus:
        # TODO: Make sure this factor of 2 is correct. My reasoning is
        # that the electron and positron are distinguishable and so you
        # need to integrate theta_L over (0, 2pi).
        lines_contrib = pws[0] * 2. / (eng_s * beta)
