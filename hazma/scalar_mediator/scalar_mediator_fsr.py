        ...
        if t_lim1(s, 0.0, mpi, mpi, Q) < t < t_lim2(s, 0.0, mpi, mpi, Q):
            ...
            msqrd = (-4 * qe**2 * (mpi**4 * s + s * t * (-Q**2 + s + t) +
                ...
                (6561. * fpi**4 * mrho**2 * mrhoT**4 *
                 (s**2 + mrho**2 * (rho_width**2 - 2. * s) + mrho**4) *
                 (mpi**2 - t)**2 * (mpi**2 + Q**2 - s - t)**2 * vh**4 *
                 (3 * vh + 3 * gsff * vs + 2 * gsGG * vs)**2 *
                 (9 * vh + 4 * gsGG * vs)**2)

            ret_val = msqrdX * msqrd
