...
def __msqrd_xx_to_s_to_kkg(Q, s, t, params):
    """
    Returns the squared matrix element for two fermions annihilating
    into two charged kaons and a photon.
    ...
    s : double
        Mandelstam variable associated with the photon, defined as
        (P-pg)^2, where P = pkp + pkm + pg.
    t : double
        Mandelstam variable associated with the charged kaon, defined as
        (P-ppip)^2, where P = pkp + pkm + pg.
    params: namedtuple
        Namedtuple of the model parameters.
    ...
    if 2. * mk < Q and 4. * mk**2 < s < Q**2 and 2. * mx < Q:
        if t_lim1(s, 0.0, mk, mk, Q) < t < t_lim2(s, 0.0, mk, mk, Q):

            ret_val = -(gsxx * qe**2 *
                        (mk**4 * s + s * t * (-Q**2 + s + t) +
                         mk**2 * (Q**4 - Q**2 * s - 2 * s * t)) *
                        (fv * gsGG * gv * (mk**2 - t) *
                         (mk**2 + Q**2 - s - t) *
                            (3 * vh + 3 * gsff * vs + 2 * gsGG * vs) *
                            (9 * vh + 4 * gsGG * vs) *
                            (9 * mrhoT * s * vh + 4 * mrho**3 *
                             (9 * vh + 2 * gsGG * vs) -
                             3 * mrho * s * (9 * vh + 2 * gsGG * vs)) -
                            6 * fpi**2 * mrho * mrhoT**2 * (mrho**2 - s) * vh *
                            (-729 * gsff * mk**2 * vh**2 +
                             128 * gsGG**3 * mk**2 * vs**2 +
                             36 * gsGG**2 * vs *
                             (6 * mk**2 * vh - 3 * Q**2 * vh +
                              2 * mk**2 * (vh - 8 * gsff * vs)) -
                             162 * gsGG * vh *
                             (-2 * mk**2 * (vh + gsff * vs) + Q**2 *
                              (vh + gsff * vs) +
                                 mk**2 * (3 * vh + 10 * gsff * vs))))**2) / \
                (6561. * fpi**4 * mrho**2 * mrhoT**4 *
                 (mrho**4 + mrho**2 * (rho_width**2 - 2 * s) + s**2) *
                 (-ms**2 + s)**2 * (mk**2 - t)**2 * (mk**2 + Q**2 - s - t)**2 *
                 vh**4 * (3 * vh + 3 * gsff * vs + 2 * gsGG * vs)**2 *
                 (9 * vh + 4 * gsGG * vs)**2)
...
def __dnde_xx_to_s_to_kkg(eng_gam, Q, params):
    """Unvectorized dnde_xx_to_s_to_kkg"""
    mx = params.mx
    ...
    if 2. * mk < Q and 4. * mk**2 <= s <= Q**2 and 2. * mx < Q:
        ...
        int_val, _ = t_integral(s, 0., mk, mk, Q, mat_elem_sqrd)
...
def dnde_xx_to_s_to_kkg(eng_gams, Q, params):
    """
    Returns the gamma ray energy spectrum for two fermions annihilating
    into two charged kaons and a photon.
    ...
    eng_gam : numpy.ndarray or double
        Gamma ray energy.
    Q : double
        Center of mass energy, or sqrt((pkp + pkm + pg)^2).

    Returns
    -------
    Returns gamma ray energy spectrum for
    :math:`\chi\bar{\chi}\to\K^{+}\K^{-}\gamma` evaluated at the gamma
    ray energy(ies).

    """

    if hasattr(eng_gams, '__len__'):
        return np.array([__dnde_xx_to_s_to_kkg(eng_gam, Q, params)
                         for eng_gam in eng_gams])
    else:
        return __dnde_xx_to_s_to_kkg(eng_gams, Q, params)
