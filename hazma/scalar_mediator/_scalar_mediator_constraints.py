...
class ScalarMediatorConstraints:
    ...
    def width_B_k_s(self):
        if self.ms <= mB - mk:
            ...
            # FCNC couplings
            hSsb = (3. * np.sqrt(2) * (msq + mbq) * GF * mtq**2 *
                    Vts.conjugate() * Vtb / (32. * np.pi**2 * vh))

            return (1. / (16. * np.pi * mB**3) *
                    ((mB**2 - mk**2) / (mbq - msq))**2 *
    ...
    def constraint_B_k_invis(self):
        ...
        if np.any([s[0] <= ms**2 <= s[1] for s in B_k_invis_obs.s_bounds]):
            ...
            pr_invis = np.exp(-B_k_invis_obs.r_max * cm_to_inv_MeV *
                              width_s * ms / ps)
    ...
    def width_k_pi_s(self):
        if self.ms < mk - mpi:
            # FCNC couplings
            hSsd = (3. * np.sqrt(2) * (msq + mdq) * GF * mtq**2 *
                    Vts.conjugate() * Vtd / (32. * np.pi**2 * vh))

            return (1. / (16. * np.pi * mk**3) * ((mk**2 - mpi**2) /
                                                  (msq - mdq))**2 *
    ...
    def width_kl_pi0_s(self):
        if self.ms < mkl - mpi0:
            ...
            return (1. / (16. * np.pi * mkl**3) * ((mkl**2 - mpi0**2) /
                                                   (msq - mdq))**2 *
                    hSsd.imag**2 * self.gsff**2 *
    ...
    def constraint_kl_pi0_mu_mu(self):
        ...
        if np.any([s[0] <= ms**2 <= s[1] for s in kl_pi0_mu_mu_obs.s_bounds]):
            ...
            # Compute the contribution to the mu mu decay width
            width_contr = (self.width_kl_pi0_s() * widths_s["mu mu"] /
                           width_s * pr_vis)
    ...
    def width_B_xs_s(self):
        ...
        if self.ms <= mB - mk:  # since K^+ is the lightest strange meson
            ...
            val = (1. / (8. * np.pi) * (mbq**2 - self.ms**2)**2 / mbq**3 *
                   abs(hSsb)**2 * self.gsff**2)
    ...
    def constrain_beam_dump(self, bd_params=bd_charm):
        ...
        br_pN_s = (bd_params.br_pN_k * (0.5 * br_k_pi_s + 0.25 * br_kl_pi0_s) +
        ...
        gamma_beta = np.sqrt((bd_params.mediator_energy / self.ms)**2 - 1.)
