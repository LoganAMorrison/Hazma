...
from hazma.vector_mediator.form_factors.utils import (MK0_GEV, MKP_GEV,
                                                      MPI_GEV, breit_wigner_fw,
                                                      breit_wigner_gs,
                                                      breit_wigner_pwave,
                                                      dhhatds, gamma_generator,
                                                      h, hhat)
...
class FormFactorKKParameters:
    ...
    omega_width: np.ndarray
    omega_coup: np.ndarray
...
def _compute_h_parameters(
    ...
    m: float
    """
    hres: np.ndarray = hhat(mass ** 2, mass, width, m, m)  # type: ignore
    dh: np.ndarray = dhhatds(mass, width, m, m)  # type: ignore
    h0: np.ndarray = h(0.0, mass, width, m, m, dh, hres)  # type: ignore
    return hres, dh, h0


def compute_kk_form_factor_parameters(
        n_max: int = 200
) -> FormFactorKKParameters:
    """
    ...
    rho_masses = np.array([0.77549, 1.5206995754050117,
                           1.7409719246639341, 1.9922811314327789])
    rho_widths = np.array([0.1494, 0.21341728317817743,
                           0.08412224414791908, 0.2899733272437917])
    # omega parameters
    omega_mag = np.array([1.3653229680598022, -0.02775156567495144,
                          -0.32497165559032715, 1.3993153161869765])
    ...
    # phi parameters
    phi_mag = np.array([0.965842498579515, -0.002379766320723148,
                        -0.1956211640216197, 0.16527771485190898])
    phi_phase = np.zeros(len(phi_mag), np.float64)
    phi_masses = np.array([1.0194209171596993,
                           1.594759278457624, 2.156971341201067])
    phi_widths = np.array([4.252653332329334e-3,
                           0.028741821847408196, 0.6737556174184005])
...
def form_factor_kk(
    ...
    # Rho exchange
    fk = ci1 * params.rho_coup * breit_wigner_gs(
        ss,
        params.rho_mass,
        params.rho_width,
        MPI_GEV,
    ...
    # Omega exchange
    fk += 1.0 / 6.0 * ci0 * params.omega_coup * breit_wigner_fw(
        ss,
        params.omega_mass,
        params.omega_width,
        reshape=True,
    ...
    # Phi-exchange
    phi_terms = 1.0 / 3.0 * cs * params.phi_coup * breit_wigner_pwave(
        ss,
        params.phi_mass,
        params.phi_width,
        mk,
