from ..decay_helper_functions cimport decay_electron
...
cdef double MASS_E = 0.510998928
...
cdef double decay_spectra_point(double eng_gam, double eng_mu):
    ...
    """
    return decay_electron.CSpectrumPoint(eng_gam, eng_mu)
...
cdef np.ndarray decay_spectra(np.ndarray eng_gam, double eng_mu):
    ...
    Returns
        ...
        `eng_gams` given muon energy `eng_mu`.
    """
    return decay_electron.CSpectrum(eng_gam, eng_mu)
...
def fsr(eng_gam, cme, mediator='scalar'):
    ...
    if mediator == 'scalar':
        if hasattr(eng_gam, "__len__"):
            ...
            return smvec(eng_gam, cme, MASS_E)
        return scalar_mediator_fsr.fermion(eng_gam, cme, MASS_E)
    if mediator == 'psuedo-scalar':
        if hasattr(eng_gam, "__len__"):
            psm = np.vectorize(pseudo_scalar_mediator_fsr.fermion)
            return psm(eng_gam, cme, MASS_E)
        return pseudo_scalar_mediator_fsr.fermion(eng_gam, cme, MASS_E)
    if mediator == 'vector':
        if hasattr(eng_gam, "__len__"):
            vm = np.vectorize(vector_mediator.fermion)
            return vm(eng_gam, cme, MASS_E)
        return vector_mediator.fermion(eng_gam, cme, MASS_E)
