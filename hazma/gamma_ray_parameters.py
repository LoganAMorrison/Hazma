...
e_astrogam_energy_res_rf = resource_filename(
    __name__, gr_data_dir + ("e-astrogam_energy"
                             "_resolution.dat"))
...
gc_bg_flux_fn = load_interp(
    gc_bg_model_rf, bounds_error=True, fill_value=np.nan)
...
energy_res_e_astrogam = load_interp(
    e_astrogam_energy_res_rf, fill_value="extrapolate")
...
# # # Target parameters
# COMPTEL diffuse
comptel_diffuse_target = TargetParams(
    J=3.725e28, dOmega=solid_angle(60., 0., 20.))
comptel_diffuse = FluxMeasurement(comptel_obs_rf, energy_res_comptel,
                                  comptel_diffuse_target)
# EGRET diffuse
egret_diffuse_target = TargetParams(
    J=3.79e27, dOmega=solid_angle(180., 20., 60.))
egret_diffuse = FluxMeasurement(egret_obs_rf, energy_res_egret,
                                egret_diffuse_target)
# Fermi diffuse
fermi_diffuse_target = TargetParams(
    J=4.698e27, dOmega=solid_angle(180., 8., 90.))
fermi_diffuse = FluxMeasurement(fermi_obs_rf, energy_res_fermi,
                                fermi_diffuse_target)
