...
def gamma_ray_decay(particles, cme, photon_energies,
    ...
    if hasattr(photon_energies, '__len__'):
        return gamma(particles, cme, photon_energies, mat_elem_sqrd=mat_elem_sqrd,
                     num_ps_pts=num_ps_pts, num_bins=num_bins, verbose=verbose)
    return gamma_point(particles, cme, photon_energies, mat_elem_sqrd,
                       num_ps_pts, num_bins)
...
def gamma_ray_fsr(isp_masses, fsp_masses, cme,
    ...
    r"""Returns the FSR spectrum from a set of particles given matrix elements.

    Returns FSR spectrum from a set of particles. This is done by
