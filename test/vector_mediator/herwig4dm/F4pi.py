# Libraries to load
import math,random,numpy,Resonance,alpha,glob,os
...
def sigmaOmega(Q2) :
    ...
    pre = alpha.alphaEM(Q2)**2*2./3./Q2**2*(2.*math.pi)**6*Resonance.gev2nb
    return pre*omega_interpolator(Q)*(res*res.conjugate()).real/(1.-br_omega_pi_gamma)
...
def phaseSpace1(m0,m1,m2,m3,m4,M12,G12,M34,G34) :
    if(random.random()<0.5) :
        ...
        m122 = max(m12min**2,min(m12max**2,M12**2+M12*G12*math.tan(rho)))
        m12=math.sqrt(m122)
    (q1,q2) = twoBodyDecay(numpy.array([m0,0.,0.,0.]),m0,m12,m34)
...
def weight1(m0,m1,m2,m3,m4,m12,m34,M12,G12,M34,G34) :
    ...
    wgt = 0.5*(wgt1+wgt2)
    # phase-space bits
    wgt *= m0 *8.*math.pi**2/pStar(m0,m12,m34)
    wgt *= m12*8.*math.pi**2/pStar(m12,m1,m2)
    wgt *= m34*8.*math.pi**2/pStar(m34,m3,m4)
    return wgt*(2.*math.pi)**3
...
def phaseSpace2(m0,m1,m2,m3,m4,M234,G234,M34,G34) :
    ...
    (p1,q234) = twoBodyDecay(numpy.array([m0,0.,0.,0.]),m0,m1,m234)
    m34min=m3+m4
    m34max=m234-m2
    rhomin = math.atan2( m34min**2 - M34**2, M34*G34)
    rhomax = math.atan2( m34max**2 - M34**2, M34*G34)-rhomin
...
def weight2(m0,m1,m2,m3,m4,m234,m34,M234,G234,M34,G34) :
    ...
    wgt *= m0 *8.*math.pi**2/pStar(m0,m1,m234)
    # weight for (234) -> 2 (34)
    m34min=m3+m4
    m34max=m234-m2
    rhomin = math.atan2( m34min**2 - M34**2, M34*G34)
    ...
    wgt *= m234*8.*math.pi**2/pStar(m234,m2,m34)
    # weight for (34) -> 3 4
    wgt *= m34*8.*math.pi**2/pStar(m34,m3,m4)
    return wgt*(2.*math.pi)**3
...
def twoBodyDecay(p0,m0,m1,m2) :
    ctheta = 2.*random.random()-1.
    stheta = math.sqrt(1.-ctheta**2)
    phi = math.pi*2.*random.random()
    pcm = pStar(m0,m1,m2)
    p1 = numpy.array([math.sqrt(pcm**2+m1**2),pcm*math.cos(phi)*stheta,pcm*math.sin(phi)*stheta,pcm*ctheta])
...
def BW3(Q2,M,G) :
    return M**2/complex(M**2-Q2,-G*M**2*math.sqrt(max(0.,((Q2-4.*mpip**2)/(M**2-4.*mpip**2))**3)/Q2))

def Brho(Q2) :
    beta = -0.145
    return 1./(1.+beta)*(BW3(Q2,mRho,gRho)+beta*BW3(Q2,mRho1,gRho1))

def Trho(Q2) :
    beta1=0.08
    beta2=-0.0075
    return 1./(1.+beta1+beta2)*(BW3(Q2,mRho,gRho)+beta1*BW3(Q2,mRho1,gRho1)+beta2*BW3(Q2,mRho2,gRho2))

def Bf0(Q2) :
    return mf0**2/complex(mf0**2-Q2,-gf0*mf0**2*math.sqrt(max(0.,((Q2-4.*mpip**2)/(mf0**2-4.*mpip**2)))/Q2))
...
def Grho(p1,p2,p3,p4) :
    pre = BWRR(m2(p1+p3))*(BWRR(m2(p2+p4))*dot(p1+p2+3.*p3+p4,p2-p4)+2.)
    return pre*p1

######################################
### Currents for each contribution ###
######################################
# p1,p2,p3,p4 are four vectors, that are numpy arrays with numpy.array([p1[0],p1[1],p1[2],p1[3]])
...
def Jomega(p1,p2,p3,p4) :
    ...
    s2=m2(p2+p4)
    s3=m2(p3+p4)
    pre = 2.*Resonance.BreitWignerFW(m2(p2+p3+p4),mOmega,gOmega)*g_omega_pi_rho*g_rho_pi_pi*H(s1,s2,s3)
    p2Q=dot(p2,Q)
    p3Q=dot(p3,Q)
    p4Q=dot(p4,Q)
    p1p2=dot(p1,p2)
    p1p3=dot(p1,p3)
    p1p4=dot(p1,p4)
    current = p2*(p1p4*p3Q-p1p3*p4Q) + p3*(p1p2*p4Q-p1p4*p2Q) + p4*(p1p3*p2Q-p1p2*p3Q)
    return -pre*current
...
def Ja1(Q2,p1,p2,p3,p4) :
    ...
    current = p3-p4 + p1*dot(p2,p3-p4)/msq - Q*(dot(Q,p3-p4)/Q2+dot(Q,p1)*dot(p2,p3-p4)/Q2/msq)
    return pre*current

# rho contribution
def Jrho(Q2,p1,p2,p3,p4) :
    Q=p1+p2+p3+p4
    c1 = Grho(p1,p2,p3,p4)+Grho(p4,p1,p2,p3)-Grho(p1,p2,p4,p3)-Grho(p3,p1,p2,p4) +\
         Grho(p2,p1,p3,p4)+Grho(p4,p2,p1,p3)-Grho(p2,p1,p4,p3)-Grho(p3,p2,p1,p4)
    d1 = dot(Q,c1)/Q2
...
def Frho(Q2,beta1,beta2,beta3) :
    return 1./(1+beta1+beta2+beta3)*(       BW3(Q2,mRho ,gRho )
...
def generatepm00(rootS) :
    rnd = random.random()
    if(rnd<0.25) : 
        (p01,p02,pp,pm) = phaseSpace1(rootS,mpi0,mpi0,mpip,mpip,mf0 ,gf0 ,mRho,gRho)
    elif(rnd<0.5) :
        rnd=4.*rnd-1.
        if(rnd<0.25) :
            (pm,p02,p01,pp) = phaseSpace2(rootS,mpip,mpi0,mpi0,mpip,ma1,ga1,mRho,gRho)
        elif(rnd<0.5) :
            (pm,p01,p02,pp) = phaseSpace2(rootS,mpip,mpi0,mpi0,mpip,ma1,ga1,mRho,gRho)
        elif(rnd<0.75) :
            (pp,p02,p01,pm) = phaseSpace2(rootS,mpip,mpi0,mpi0,mpip,ma1,ga1,mRho,gRho)
        else :
            (pp,p01,p02,pm) = phaseSpace2(rootS,mpip,mpi0,mpi0,mpip,ma1,ga1,mRho,gRho)
    elif(rnd<0.75) :
        rnd=4.*rnd-2.
        if(rnd<0.5) :
            (p01,pm,p02,pp) = phaseSpace1(rootS,mpi0,mpip,mpi0,mpip,mRho,gRho,mRho,gRho)
        else :
            (p02,pm,p01,pp) = phaseSpace1(rootS,mpi0,mpip,mpi0,mpip,mRho,gRho,mRho,gRho)
    else :
        rnd=4.*rnd-3.
        if(rnd<1./6.) :
            (p01,p02,pp,pm) = phaseSpace2(rootS,mpi0,mpi0,mpip,mpip,mOmega,gOmega,mRho,gRho)
        elif(rnd<1./3.) :
            (p02,p01,pp,pm) = phaseSpace2(rootS,mpi0,mpi0,mpip,mpip,mOmega,gOmega,mRho,gRho)
        elif(rnd<0.5) :
            (p01,pp,p02,pm) = phaseSpace2(rootS,mpi0,mpip,mpi0,mpip,mOmega,gOmega,mRho,gRho)
        elif(rnd<2./3.) :
            (p02,pp,p01,pm) = phaseSpace2(rootS,mpi0,mpip,mpi0,mpip,mOmega,gOmega,mRho,gRho)
        elif(rnd<5./6.) :
            (p01,pm,p02,pp) = phaseSpace2(rootS,mpi0,mpip,mpi0,mpip,mOmega,gOmega,mRho,gRho)
        else :
            (p02,pm,p01,pp) = phaseSpace2(rootS,mpi0,mpip,mpi0,mpip,mOmega,gOmega,mRho,gRho)
  
    wgt = 1./4.*weight1(rootS,mpi0,mpi0,mpip,mpip,math.sqrt(m2(p01+p02)),math.sqrt(m2(pp +pm )),mf0 ,gf0 ,mRho,gRho) + \
          1./4./6.*(weight2(rootS,mpi0,mpi0,mpip,mpip,math.sqrt(m2(p02+pp+pm)),math.sqrt(m2(pp+pm )),mOmega,gOmega,mRho,gRho) +
                    weight2(rootS,mpi0,mpi0,mpip,mpip,math.sqrt(m2(p01+pp+pm)),math.sqrt(m2(pp+pm )),mOmega,gOmega,mRho,gRho) +
                    weight2(rootS,mpi0,mpip,mpi0,mpip,math.sqrt(m2(p02+pp+pm)),math.sqrt(m2(p02+pm)),mOmega,gOmega,mRho,gRho) +
                    weight2(rootS,mpi0,mpip,mpi0,mpip,math.sqrt(m2(p01+pp+pm)),math.sqrt(m2(p01+pm)),mOmega,gOmega,mRho,gRho) +
                    weight2(rootS,mpi0,mpip,mpi0,mpip,math.sqrt(m2(p02+pp+pm)),math.sqrt(m2(p02+pp)),mOmega,gOmega,mRho,gRho) +
                    weight2(rootS,mpi0,mpip,mpi0,mpip,math.sqrt(m2(p01+pp+pm)),math.sqrt(m2(p01+pp)),mOmega,gOmega,mRho,gRho)) +\
          1./4./4.*(weight2(rootS,mpip,mpi0,mpi0,mpip,math.sqrt(m2(p02+p01+pp)),math.sqrt(m2(p01+pp)),ma1,ga1,mRho,gRho)+
                    weight2(rootS,mpip,mpi0,mpi0,mpip,math.sqrt(m2(p01+p02+pp)),math.sqrt(m2(p02+pp)),ma1,ga1,mRho,gRho)+
                    weight2(rootS,mpip,mpi0,mpi0,mpip,math.sqrt(m2(p02+p01+pm)),math.sqrt(m2(p01+pm)),ma1,ga1,mRho,gRho)+
                    weight2(rootS,mpip,mpi0,mpi0,mpip,math.sqrt(m2(p01+p02+pm)),math.sqrt(m2(p02+pm)),ma1,ga1,mRho,gRho)) +\
          1./4./2.*(weight1(rootS,mpi0,mpip,mpi0,mpip,math.sqrt(m2(p01+pm)),math.sqrt(m2(p02+pp)),mRho,gRho,mRho,gRho) +
                    weight1(rootS,mpi0,mpip,mpi0,mpip,math.sqrt(m2(p02+pm)),math.sqrt(m2(p01+pp)),mRho,gRho,mRho,gRho))
    return (1./wgt,p01,p02,pp,pm)
...
def generatepmpm(rootS) :
    ...
    if(rnd<0.5) :
        rnd *= 2.
        if(rnd<0.25) :
            (pp1,pm1,pp2,pm2) = phaseSpace1(rootS,mpip,mpip,mpip,mpip,mf0 ,gf0 ,mRho,gRho)
        elif(rnd<0.5) :
            (pp1,pm2,pp2,pm1) = phaseSpace1(rootS,mpip,mpip,mpip,mpip,mf0 ,gf0 ,mRho,gRho)
        elif(rnd<0.75) :
            (pp2,pm1,pp1,pm2) = phaseSpace1(rootS,mpip,mpip,mpip,mpip,mf0 ,gf0 ,mRho,gRho)
        else :
            (pp2,pm2,pp1,pm1) = phaseSpace1(rootS,mpip,mpip,mpip,mpip,mf0 ,gf0 ,mRho,gRho)
    else :
        rnd = 2.*rnd-1.
        if(rnd<0.125) :
            (pp1,pm1,pp2,pm2) = phaseSpace2(rootS,mpip,mpip,mpip,mpip,ma1,ga1,mRho,gRho)
        elif(rnd<0.25) :
            (pp1,pm2,pp2,pm1) = phaseSpace2(rootS,mpip,mpip,mpip,mpip,ma1,ga1,mRho,gRho)
        elif(rnd<0.375) :
            (pp2,pm1,pp1,pm2) = phaseSpace2(rootS,mpip,mpip,mpip,mpip,ma1,ga1,mRho,gRho)
        elif(rnd<0.5) :
            (pp2,pm2,pp1,pm1) = phaseSpace2(rootS,mpip,mpip,mpip,mpip,ma1,ga1,mRho,gRho)
        elif(rnd<0.625) :
            (pm1,pp1,pm2,pp2) = phaseSpace2(rootS,mpip,mpip,mpip,mpip,ma1,ga1,mRho,gRho)
        elif(rnd<0.75) :
            (pm1,pp2,pm2,pp1) = phaseSpace2(rootS,mpip,mpip,mpip,mpip,ma1,ga1,mRho,gRho)
        elif(rnd<0.875) :
            (pm2,pp1,pm1,pp2) = phaseSpace2(rootS,mpip,mpip,mpip,mpip,ma1,ga1,mRho,gRho)
        else :
            (pm2,pp2,pm1,pp1) = phaseSpace2(rootS,mpip,mpip,mpip,mpip,ma1,ga1,mRho,gRho)
    wgt = 0.125*(weight1(rootS,mpip,mpip,mpip,mpip,math.sqrt(m2(pp2+pm2)),math.sqrt(m2(pp1+pm1)),mf0 ,gf0 ,mRho,gRho)+
                weight1(rootS,mpip,mpip,mpip,mpip,math.sqrt(m2(pp2+pm1)),math.sqrt(m2(pp1+pm2)),mf0 ,gf0 ,mRho,gRho)+
                weight1(rootS,mpip,mpip,mpip,mpip,math.sqrt(m2(pp1+pm2)),math.sqrt(m2(pp2+pm1)),mf0 ,gf0 ,mRho,gRho)+
                weight1(rootS,mpip,mpip,mpip,mpip,math.sqrt(m2(pp1+pm1)),math.sqrt(m2(pp2+pm2)),mf0 ,gf0 ,mRho,gRho)) + \
          0.0625*(weight2(rootS,mpip,mpip,mpip,mpip,math.sqrt(m2(pp2+pp1+pm1)),math.sqrt(m2(pp1+pm1)),ma1,ga1,mRho,gRho)+
                  weight2(rootS,mpip,mpip,mpip,mpip,math.sqrt(m2(pp2+pp1+pm2)),math.sqrt(m2(pp1+pm2)),ma1,ga1,mRho,gRho)+
                  weight2(rootS,mpip,mpip,mpip,mpip,math.sqrt(m2(pp2+pp1+pm1)),math.sqrt(m2(pp2+pm1)),ma1,ga1,mRho,gRho)+
                  weight2(rootS,mpip,mpip,mpip,mpip,math.sqrt(m2(pp2+pp1+pm2)),math.sqrt(m2(pp2+pm2)),ma1,ga1,mRho,gRho)+
                  weight2(rootS,mpip,mpip,mpip,mpip,math.sqrt(m2(pm2+pm1+pp1)),math.sqrt(m2(pm1+pp1)),ma1,ga1,mRho,gRho)+
                  weight2(rootS,mpip,mpip,mpip,mpip,math.sqrt(m2(pm2+pm1+pp2)),math.sqrt(m2(pm1+pp2)),ma1,ga1,mRho,gRho)+
                  weight2(rootS,mpip,mpip,mpip,mpip,math.sqrt(m2(pm2+pm1+pp1)),math.sqrt(m2(pm2+pp1)),ma1,ga1,mRho,gRho)+
                  weight2(rootS,mpip,mpip,mpip,mpip,math.sqrt(m2(pm2+pm1+pp2)),math.sqrt(m2(pm2+pp2)),ma1,ga1,mRho,gRho))
    return (1./wgt,pp1,pp2,pm1,pm2)
...
def F4pi(Q2,p1,p2,p3,p4):
    ...
    for i in range(0,4) :
        ...
        if i==0 :
            ...
            f1 = c_omega*Frho(Q2,beta1_omega,beta2_omega,beta3_omega)*Jomega(Q2,p1,p2,p3,p4)
        elif i==2 :
        ...
        for j in range (0,4) :
            f2 = 0.
            if j==0 :
                f2 = c_a1   *Frho(Q2,beta1_a1,beta2_a1,beta3_a1)*Ja1(Q2,p1,p2,p3,p4)
            elif j==1 :
                f2 = c_omega*Frho(Q2,beta1_omega,beta2_omega,beta3_omega)*Jomega(Q2,p1,p2,p3,p4)
            elif j==2 :
                f2 = c_f0   *Frho(Q2,beta1_f0   ,beta2_f0   ,beta3_f0   )*Jf0(Q2,p1,p2,p3,p4)
            elif j==3 :
...
def GammaDM(mMed,mode) :
    ...
    if mode=="neutral": hadcurr = hadronic_interpolator_n(mMed)
    if mode=="charged": hadcurr = hadronic_interpolator_c(mMed)   
    # print (pre, hadcurr)
    return pre*hadcurr
