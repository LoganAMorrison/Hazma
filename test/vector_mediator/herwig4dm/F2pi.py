...
def initialize():
    ...
    for ix in range(0, len(rhoMag_)):
        rhoWgt_.append(
            rhoMag_[ix]*(math.cos(rhoPhase_[ix])+complex(0., 1.)*math.sin(rhoPhase_[ix])))
        if(ix > 0):
            rhoSum += rhoWgt_[ix]
    omegaWgt_ = omegaMag_*(math.cos(omegaPhase_) +
                           complex(0., 1.)*math.sin(omegaPhase_))
    ...
    for ix in range(0, nMax_):
        ...
        if(ix > 0):
            gamB *= ((1.-beta_+float(ix)))/float(ix)
        ...
        h0_.append(H(0., mass_[-1], width_[-1],
...
def Fpi(q2, imode):
    ...
    # print coup_[0]
    for ix in range(0, nMax_):
        term = coup_[ix]*BreitWignerGS(q2, mass_[ix], width_[ix],
                                       mpi_, mpi_, h0_[ix], dh_[ix], hres_[ix])
        # include rho-omega if needed
        if ix == 0 and imode != 0:
            term *= 1./(1.+omegaWgt_)*(1.+omegaWgt_ *
                                       BreitWignerFW(q2, omegaMass_, omegaWidth_))
        # sum
        FPI += term
    # factor for cc mode
    if(imode == 0):
...
def GammaDM(medMass):
    ...
    return 1./48./math.pi*medMass*(1-4*mpi_**2/medMass**2)**1.5*abs(temp)**2*gev2nb
...
def sigmaSM(Q2):
    ...
    temp = Fpi(Q2, 1)
    return 1./3.*math.pi*alphaEM**2/Q2*(1.-4.*mpi_**2/Q2)**1.5*abs(temp)**2*gev2nb
...
def sigmaDM(Q2):
    ...
    DMmed2 = abs(DMmed)**2
    temp = Fpi(Q2, 1)
    return 1/48./math.pi*DMmed2*Q2*(1+2*mDM_**2/Q2)*(1.-4.*mpi_**2/Q2)**1.5*abs(temp)**2*gev2nb
