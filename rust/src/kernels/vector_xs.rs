...
//! the `.pyx`. Two expressions raise a double to the power `1.5`, and
//! Cython 3's default `cpow` semantics say a `double ** double` may be
//! complex — so it compiles the *whole enclosing expression* in
//! `double _Complex` and converts back with `__Pyx_SoftComplexToDouble`,
//! which raises `TypeError` if the imaginary part is non-zero.
//! `grep -c SoftComplexToDouble` over the generated C finds exactly two
//! call sites, `__sigma_xx_to_v_to_pipi` and `__sigma_xx_to_v_to_pi0v`,
//! and the shipped object code confirms it: both call `cpow` and both
//! call compiler-rt's `___divdc3`, while the other three kernels call
//! neither.
//!
//! Neither routine agrees with its real-arithmetic spelling, so this is
//! not a detail that can be transcribed away. Measured on the capturing
//! platform (macOS/arm64, Apple clang, libSystem libm) over 3.7M
//! logarithmically spaced arguments:
//!
//! * `cpow(t + 0i, 1.5 + 0i)` differs from `pow(t, 1.5)` at 90% of
//!   arguments, by up to **9.0e-15** relative, and from `t·√t` by up to
//!   9.1e-15 — but equals `exp(1.5·log(t))` **at every one of them**,
//!   which is `cexp(w·clog(z))` evaluated at zero imaginary part. That
//!   is what [`soft_complex_pow_1_5`] computes.
//! * `(a + 0i)/(c + 0i)` differs from `a/c` at 32% of arguments, by up to
//!   4.0e-16 — one ulp — because `__divdc3` divides through a scaled
//!   `(a·c′)/(c′·c′)` rather than dividing once. That is
...
fn complex_quotient_real_denominator(a: f64, c: f64) -> Result<f64, NonRealResult> {
    let logbw = if c == 0.0 {
        f64::NEG_INFINITY
    } else if c.is_finite() {
...
mod tests {
    use super::{
        ALPHA_EM, FPI, ME, MMU, MPI, MPI0, NonRealResult, PI_4, PI_5,
        complex_quotient_real_denominator, ilogb_finite, propagator, scalbn, sigma_xx_to_all,
        sigma_xx_to_v_to_ff, sigma_xx_to_v_to_pi0g, sigma_xx_to_v_to_pi0v, sigma_xx_to_v_to_pipi,
        sigma_xx_to_vv, soft_complex_pow_1_5, thermal_cross_section,
    };
