...
##
## Parameter set for hadronic current
##
...
def resetParameters(gDM, mDM, mMed, wMed, cMedu, cMedd, cMeds):
    ...
    wMed_ = wMed
    cI1_ = cMedu - cMedd
    # taken out since omega mixes with rho and is not coupling to dark mediator
    # cI0_ = cMedu+cMedd
...
def initialize():
    ...
    for ix in range(0, len(rhoMag_)):
        rhoWgt_.append(
            rhoMag_[ix]
            * (
                math.cos(rhoPhase_[ix])
                + complex(0.0, 1.0) * math.sin(rhoPhase_[ix])
            )
        )
        if ix > 0:
            rhoSum += rhoWgt_[ix]
    ...
    for ix in range(0, nMax_):
        ...
        h0_.append(
            H(0.0, mass_[-1], width_[-1], mpi_, mpi_, dh_[-1], hres_[-1])
        )
    ...
    for ix in range(0, nMax_):
        ...
        if ix == 0 and imode != 0:
            term *= (
                ...
                * (
                    1.0
                    + omegaWgt_ * BreitWignerFW(q2, omegaMass_, omegaWidth_)
                )
            )
        # sum
        FPI += term
