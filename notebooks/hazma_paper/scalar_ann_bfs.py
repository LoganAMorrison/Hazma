...
for idx in range(2):
    axes[1, idx].set_xlabel(r"$m_\chi$ (MeV)")
    axes[idx, 0].set_ylabel("Ann. BF")

# Label couplings
axes[0, 1].text(260, 7e-3, r"$\sin\theta = 0.1$", va="center", fontsize=10)
axes[1, 1].text(
    260,
    7e-3,
    r"$g_{S\Psi} = 0.1$" + "\n" + r"$Q_\Psi = 1$" + "\n" + r"$m_\Psi = 1$ GeV",
...
axes[0, 0].set_title(r"$m_S = 200$ MeV", fontsize=10)
axes[0, 1].set_title(r"$m_S > 500$ MeV", fontsize=10)
...
for col in range(2):
    ...
    axes[0, col].text(145, 5e-4, r"$\pi^+ \pi^-$", color=get_color("pi pi"), fontsize=8)
    axes[0, col].text(
        180,
        1.1e-1,
        r"$\pi^0 \pi^0$",
...
for col in range(2):
    ...
    axes[1, col].text(145, 3e-3, r"$\pi^+ \pi^-$", color=get_color("pi pi"), fontsize=8)
    axes[1, col].text(
        170,
        1.1e-1,
        r"$\pi^0 \pi^0$",
