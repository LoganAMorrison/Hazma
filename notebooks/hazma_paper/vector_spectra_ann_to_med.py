...
vms = [KineticMixing(1., mv=1., gvxx=1., eps=0.1),
       QuarksOnly(1., mv=1., gvxx=1., gvuu=1., gvdd=-1., gvss=0.),
       QuarksOnly(1., mv=1., gvxx=1., gvuu=1., gvdd=1., gvss=0.)]
    ...
    for col in range(n_subplot_cols):  # loop over DM masses
        ...
        for mv in mvs:
            if mv <= e_cm / 2:
                ...
                ax.loglog(
                    e_gams, vm.spectra(e_gams, e_cm)["v v"], color=cmap(vm.mv))
...
axes[1, -1].text(5e2, 1e-2, r"$g_{Vuu} = 1$" + "\n" + r"$g_{Vdd} = -1$",
                 va="center", fontsize=10)
axes[2, -1].text(5e2, 1e-2, r"$g_{Vuu} = 1$" + "\n" + r"$g_{Vdd} = 1$",
                 va="center", fontsize=10)
