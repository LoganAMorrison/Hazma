...
| ruff, changed files vs `origin/master` | per-file counts, both trees | **80 → 79.** Every remaining error is in the two wrappers, which are pre-existing trunk debt ([its follow-up](../../../../docs/followups/done/preflight-isort-ruff-red-on-trunk.md)); the new module and all nine touched test/parity files are at **0**, and the scalar wrapper's isort debt was fixed in passing. No regression, one improvement. |
