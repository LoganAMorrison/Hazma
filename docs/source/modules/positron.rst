...
``positron_spectra.py``, for computing the electron/positron spectra from
decays of :math:`\pi^{\pm}` and :math:`\mu^{\pm}`. As in the ``decay.py``
module, the ``positron_spectra.py`` module allows users to compute the
...
energies is identical to the procedure used for ``decay.py``. In addition
