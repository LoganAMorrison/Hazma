...
:py:class:`Rambo` have the a shape of `(4, nfsp, n)`
...
.. warning::
   ...
   isn't the case, then this class is not suitable. For example, the squared
...
To generate energy and invariant mass distributions, use:
...
.. code-block:: python
   ...
   tb = phase_space.ThreeBody(MMU, [0.0, 0.0, 0.0], msqrd=msqrd)

   energy_dists = tb.energy_distributions(nbins=30)
   inv_mass_dists = tb.invariant_mass_distributions(nbins=30)

We can plot these distributions using the following:
