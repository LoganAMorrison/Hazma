...
| --- | --- | --- |
| [`thermal_cross_section` returns its integrator's initial estimate](done/thermal-cross-section-quadrature-never-converges.md) | done | `parity-pinned-defect-repair` Task 13, as roster entry `B5`. `epsabs = 0` at all four call sites — both Rust kernels plus the two pure-Python ones the port never touched — and `THERMAL_LIMIT` raised from 50 to 100, without which the criterion could not be reached at 33 of the 540 corpus positions. **The file's own 0.5%–5% estimate was measured at one model point and understated the defect**: swept over all 570 pinned corpus positions against scipy's QUADPACK at `epsrel = 1e-12`, the shipped ⟨σv⟩ is wrong by up to 100%, and `relic_density` moves by −99.9% to +2.4% over the six pinned model points. The corpus keeps its pre-repair arrays; the 539 moved positions are declared through a new `Reference` relation in `test/parity/deltas.py`, since the stored value is the true one unresolved rather than shifted by a term the physics names. Recorded in `CHANGELOG.md`. |
