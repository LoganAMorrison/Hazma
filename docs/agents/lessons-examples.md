# Review-lessons ledger: worked examples

The evidence behind each one-line rule in [`lessons.md`](lessons.md),
one `###` section per `[class]`, in the ledger's order. Each section is
the full ledger entry as it stood before the 2026-08-21 split
([ADR-0002](../adrs/ADR-0002-read-phase-learnings-not-closed-task-notes.md)) — the
rule, the PR(s), what the PR did and what review caught — moved here
verbatim, plus anything appended since. Read a section when its
one-line rule is not enough to act on; do not read this file front to
back on a task, and do not cite it as the rule — `lessons.md` is the
contract.

**Append** (`review-respond`, the lessons step): a new class gets a
`### <class>` heading here and a one-liner in `lessons.md`, in the same
commit; a recurrence of an existing class appends a paragraph under its
heading here and adds the PR to the citation list there. Every example
cites a real PR.

## Examples

### ported-file-stale-reference
...
### artifact-inventory-depends-on-cwd-state
...
### docstring-section-not-reconciled
...
- [touched-doc-inherits-its-citations] Editing *any* line of a durable doc
  ...
  merely stale: a `_pion.pyx` cited by bare basename and line range
  resolved fine while three files of that basename existed, and became a
  hard `AMBIGUOUS` the moment the PR deleted the one it meant, leaving two
  candidates and no way to pick. Note this very sentence had to drop the
  line numbers to stay checkable — **a ledger entry about a bad citation
  is itself a citation** (PR #74 — the task note's sweep block listed the
  `rg` half of the line-number sweep and omitted
  `check_doc_citations.py`, so nothing
...
### measured-tree-vs-imported-module
...
### bound-parameter-sized-the-allocation
...
### sweep-excluded-the-canonical-directory
...
### deadline-bound-to-the-wrong-artifact
